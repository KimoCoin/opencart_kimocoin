...
class ControllerPaymentKimoCoin extends Controller {
	...
	private function validate() {
        ...
        if (!$this->request->post['kimocoin_rpc_username']) {
            $this->error['username'] = $this->language->get('error_username');
        }
        if (!$this->request->post['kimocoin_rpc_address']) {
            $this->error['address'] = $this->language->get('error_address');
        }
        if (!$this->request->post['kimocoin_rpc_password']) {
            $this->error['password'] = $this->language->get('error_password');
        }
        if (!$this->request->post['kimocoin_rpc_port']) {
            $this->error['port'] = $this->language->get('error_port');
        }
        if (!$this->request->post['kimocoin_prefix']) {
            $this->error['prefix'] = $this->language->get('error_prefix');
        }
        if (!$this->request->post['kimocoin_countdown_timer']) {
	...
	public function index() {
        ...
        $this->data['entry_show_kmc']         = $this->language->get('entry_show_kmc');
        $this->data['entry_kmc_decimal']         = $this->language->get('entry_kmc_decimal');
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/kimocoin', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );
		...
		$rows = $result->rows;
		$max = $result->num_rows;
		$kimocoin_total_in_db = 0;
		$kimocoin_address_in_db = 0;
		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "kimocoin_total") {
				$kimocoin_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "kimocoin_address") {
				$kimocoin_address_in_db = 1;
			}
		}
		if(!$kimocoin_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD kimocoin_total DOUBLE AFTER currency_value;");
		}
		if(!$kimocoin_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD kimocoin_address VARCHAR(34) AFTER kimocoin_total;");
		}
        ...
        if (isset($this->request->post[$this->payment_module_name.'_show_kmc'])) {
			$this->data[$this->payment_module_name.'_show_kmc'] = $this->request->post[$this->payment_module_name.'_show_kmc'];
		} else {
			$this->data[$this->payment_module_name.'_show_kmc'] = $this->config->get($this->payment_module_name.'_show_kmc');
		}

        if (isset($this->request->post[$this->payment_module_name.'_kmc_decimal'])) {
			$this->data[$this->payment_module_name.'_kmc_decimal'] = $this->request->post[$this->payment_module_name.'_kmc_decimal'];
		} else {
			$this->data[$this->payment_module_name.'_kmc_decimal'] = $this->config->get($this->payment_module_name.'_kmc_decimal');
		}
